...
morto_giorno=[0 for _ in range(giocatori)]
for simulazione in range(tot_simulazioni):
    ...
    while not finito:
        ...
        if indice_criceto not in vivi: 
            ...
            c+=1
            mortodigiorno=balottaggio()
            if mortodigiorno:

                break

print("Risultati dopo",morto_giorno,"simulazioni:")
for i in range(giocatori):
    if morto_giorno[i]>0:   
        print(f"il criceto è morto alla notte {i} nel {criceto_morto[i]/morto_giorno[i]*100}% delle volte")
